...
describe('assertFile', () => {
  describe('assertFileExists', () => {
    ...
    it('should throw an error if no files are provided', () => {
      expect(() => assertFileExists(undefined)).toThrow(
        'At least one picture file is required',
      );
      expect(() => assertFileExists([] as FileUpload[])).toThrow(
        'At least one picture file is required',
      );
      expect(() => assertFileExists([])).toThrow(
        'At least one picture file is required',
      );
    });
  });
  describe('assertValidImageFile', () => {
    ...
    it('should throw for invalid file types', () => {
      ...
      expect(() => assertValidImageFile(file)).toThrow(
