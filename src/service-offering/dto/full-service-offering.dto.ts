...
export class FullServiceOfferingDto {
    ...
    @ApiProperty({ example: 'FIXED', description: 'Pricing type (e.g. FIXED, HOURLY)' })
    priceType: string;

    @ApiProperty({ example: 120 })
    durationMinutes: number;

    @ApiProperty({ example: 'MINUTES', required: false, description: 'Unit of duration' })
    durationUnit?: string;
    ...
    @ApiProperty({ example: 3, description: 'Number of Cars the service can handle at once' })
    capacity: number;

    @ApiProperty({ example: new Date().toISOString(), description: 'Service creation timestamp' })
    createdAt: Date;
}