...
describe('BookingService', () => {
    ...
    describe('create()', () => {
        ...
        it('should create a booking and return response DTO', async () => {
            ...
            jest.spyOn(bookingTransactionService, 'createBookingWithTransaction').mockResolvedValue(mockBooking);
            jest.spyOn(require('../utils/booking.mapper').BookingMapper, 'toResponseDto').mockReturnValue(mockResponse);

            const result = await service.create(createDto, 5);

            expect(result).toEqual(mockResponse);
            expect(bookingTransactionService.createBookingWithTransaction).toHaveBeenCalledWith(createDto, 5);
        });

        it('should throw BadRequestException for BookingValidationException', async () => {
            jest.spyOn(bookingTransactionService, 'createBookingWithTransaction').mockRejectedValue(
                new BookingValidationException('Invalid booking')
            );
        ...
        it('should throw ConflictException for BookingConflictException', async () => {
            jest.spyOn(bookingTransactionService, 'createBookingWithTransaction').mockRejectedValue(
                new BookingConflictException('Conflict')
            );
    ...
    describe('findUserBookings()', () => {
        it('should fetch bookings for a user', async () => {
            ...
            jest.spyOn(bookingDtoService, 'mapQueryDtoToOptions').mockReturnValue(options);
            jest.spyOn(bookingQueryService, 'findBookingsWithPagination').mockResolvedValue({ bookings: [], total: 0 });
            jest.spyOn(bookingDtoService, 'createPaginatedResponse').mockReturnValue(mockResponse);

            const result = await service.findUserBookings(query, 42);

            expect(result).toEqual(mockResponse);
            expect(bookingDtoService.mapQueryDtoToOptions).toHaveBeenCalledWith(query);
        });
    });
    ...
    describe('findBusinessBookings()', () => {
        it('should fetch bookings for a business and verify ownership', async () => {
            ...
            const result = await service.findBusinessBookings(query, businessId, userId);

            expect(businessService.isOwnedByUser).toHaveBeenCalledWith(userId, businessId);
            expect(result).toEqual(mockResponse);
        });
    });
});
