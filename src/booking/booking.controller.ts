...
export class BookingController {
  ...
  @Post()
  @ApiOperation({ summary: 'Create a new booking', description: 'Creates a booking for authenticated users or guest users with provided contact information' })
  @ApiConsumes('application/json')
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Booking created successfully', type: BookingResponseDto, })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Invalid booking data or validation errors', })
  @ApiResponse({ status: HttpStatus.CONFLICT, description: 'Booking conflicts with existing reservation', })
  ...
  @ApiOperation({ summary: 'Get bookings for a specific business', description: 'Retrieves all bookings for a business (business owners and managers only)' })
  @ApiParam({ name: 'businessId', description: 'Business ID', type: 'number' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Business bookings retrieved successfully', })
  @ApiResponse({ status: HttpStatus.FORBIDDEN,  description: 'Access denied - business owner/manager role required' })
  ...
  async findBusinessBookings(
      ...
      this.logger.log(`Fetching bookings for business: ${businessId} by user: ${req.user.id}`);
      
      return this.bookingService.findBusinessBookings(query, businessId, req.user.id);
  }

  @Get('user/my-bookings')
  @ApiOperation({ summary: 'Get current user\'s bookings', description: 'Retrieves all bookings for the authenticated user across all businesses' })
  @ApiResponse({ status: HttpStatus.OK, description: 'User bookings retrieved successfully', type: PaginatedBookingResponseDto })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  async getMyBookings(
    @Query() query: QueryBookingDto,
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Update a booking', description: 'Updates booking details like status or special requests' })
  ...
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Access denied or invalid status transition', })
  ...
  @Delete(':id')
  @ApiOperation({ summary: 'Cancel a booking', description: 'Cancels a booking by setting its status to cancelled' })
  @ApiParam({ name: 'id', description: 'Booking ID', type: 'number' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Booking cancelled successfully', type: BookingResponseDto, })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Booking not found', })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Access denied - not your booking', })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  async cancel(
    @Param('id', ParseIntPipe) id: number,
  ...
  @Post(':id/confirm')
  @ApiOperation({ summary: 'Confirm a booking', description: 'Confirms a booking (business owners only)' })
  @ApiParam({ name: 'id', description: 'Booking ID', type: 'number' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Booking confirmed successfully', type: BookingResponseDto, })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Booking not found', })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Access denied - business owner role required', })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Booking cannot be confirmed in current status', })
  @ApiBearerAuth()
