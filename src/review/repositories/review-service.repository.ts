...
export class ReviewServiceRepository {
  ...
  async createMultiple(reviewServicesData: Partial<ReviewedService>[]): Promise<ReviewedService[]> {
    try {
      const reviewServices = this.reviewServiceRepository.create(reviewServicesData);
      return await this.reviewServiceRepository.save(reviewServices);
    } catch (error) {
      this.logger.error(`Failed to create review services: ${error.message}`, error.stack);
      throw error;
    }
  }
}