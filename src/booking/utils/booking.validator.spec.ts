...
describe('BookingValidator', () => {
  describe('validateGuestBookingData', () => {
    it('should throw if guest info is missing and user is unauthenticated', () => {
      ...
      expect(() => BookingValidator.validateGuestBookingData(dto)).toThrow(
    ...
    it('should not throw if user is authenticated', () => {
      ...
      expect(() =>
        BookingValidator.validateGuestBookingData(dto, 123),
      ).not.toThrow();
    });
    ...
    it('should not throw if guest info is provided for guest booking', () => {
      ...
      expect(() =>
        BookingValidator.validateGuestBookingData(dto),
      ).not.toThrow();
    });
  });
  ...
  describe('validateBookingTime', () => {
    it('should throw if date is in the past', () => {
      ...
      expect(() => BookingValidator.validateBookingTime(pastDate)).toThrow(
        BookingValidationException,
      );
    });
    ...
    it('should not throw if date is in the future', () => {
      ...
      expect(() =>
        BookingValidator.validateBookingTime(futureDate),
      ).not.toThrow();
    });
  });
  ...
  describe('validateBookingAccess', () => {
    ...
    it('should throw if user is not owner or business owner', () => {
      expect(() => BookingValidator.validateBookingAccess(booking, 2)).toThrow(
        ForbiddenException,
      );
    });
    ...
    it('should not throw if user is the booking owner', () => {
      expect(() =>
        BookingValidator.validateBookingAccess(booking, 1),
      ).not.toThrow();
    });

    it('should not throw if business owner is accessing', () => {
      expect(() =>
        BookingValidator.validateBookingAccess(booking, undefined, true),
      ).not.toThrow();
    });
  });
