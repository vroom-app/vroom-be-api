...
export function subtractInterval(
  ...
  const overlapping = intervals.filter(
    (interval) =>
      interval.start < toSubtract.end && interval.end > toSubtract.start,
  );
  ...
  for (const currentInterval of intervals) {
    if (currentInterval !== intervalToDivide) {
      ...
      if (intervalToDivide.start < toSubtract.start) {
        result.push({
          start: intervalToDivide.start,
          end: Math.min(intervalToDivide.end, toSubtract.start),
        });
      }
      if (intervalToDivide.end > toSubtract.end) {
        result.push({
          start: Math.max(intervalToDivide.start, toSubtract.end),
          end: intervalToDivide.end,
        });
      }
    }
  }
...
export function getDefaultStartDateAndDays(): {
  startDateStr: string;
  days: number;
} {
