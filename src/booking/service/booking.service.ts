...
export class BookingService {
  ...
  public async create(
    ...
    try {
      const savedBooking =
        await this.bookingTransactionService.createBookingWithTransaction(
          createBookingDto,
          userId,
        );
  ...
  public async findBusinessBookings(
    ...
    this.logger.log(
      `Fetching bookings for business: ${businessId} by user: ${userId}`,
    );
  ...
  private async executeBookingQuery(
    ...
    const { bookings, total } =
      await this.bookingQueryService.findBookingsWithPagination(
        options,
        context,
      );

    return this.bookingDtoService.createPaginatedResponse(
      bookings,
      total,
      options,
      context,
