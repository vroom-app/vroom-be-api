...
export class BusinessReviewService {
    ...
    async updateBusinessRating(businessId: string) {
        ...
        this.logger.log(`Business rating updated for business ID: ${businessId}, new average rating: ${business.averageRating}, review count: ${business.reviewCount}`);
    }

    private async syncBusinessWithSearchEngine(savedBusiness: Business) {
        this.logger.log(`Syncing business ratings with search engine ${savedBusiness}`);
